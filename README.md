...
| 大纲 / 文件侧边栏 | 大纲视图速览文档结构；文件视图浏览并切换同目录 Markdown |
| 文件过滤 | 文件视图下用「眼镜」按钮只看可打开文件，及其后代含可打开文件的目录（单个文件打开时不显示） |
| 分屏预览 | 左编辑 / 右预览；支持仅编辑、仅预览（默认纯预览） |
| 预览区高亮 | 在渲染结果上框选文字即可高亮，标注写回源码 `<mark>`；整段选中已高亮块可取消；预览区其余为只读 |
...
- **预览区「框选-高亮」重构**：通过 DOMPurify 放行 `data-source-line` 行锚点 + 渲染/源码偏移恒等映射精确定位，支持跨多行、重复文本、加粗/斜体内部选区，并支持整段取消高亮；高亮失败提示改为两行居中（主提示 + `G码--原因`）。
- **预览区改为只读（高亮除外）**：预览区不再做 WYSIWYG 回写源码，仅保留「高亮」作为唯一可写操作，从源头避免脏 DOM 累加与格式重排。
- **左侧功能区重设计**：大纲与文件双视图切换；文件视图新增「眼镜」过滤按钮，只看可打开文件及其含可打开文件的目录，过滤状态持久化且不重新扫描。
- **顶部视图模式**：顺序调整为「纯编辑 → 分屏 → 纯预览」，默认纯预览且不再持久化记忆。

详细变更见 [CHANGELOG.md](./CHANGELOG.md)，开发过程见 [DEVLOG.md](./DEVLOG.md)。
